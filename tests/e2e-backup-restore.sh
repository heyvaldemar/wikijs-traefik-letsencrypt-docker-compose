#!/bin/bash
# End-to-end tests for the wikijs-traefik-letsencrypt-docker-compose backup + restore flow.
#
...
COMPOSE_PROJECT_NAME="${COMPOSE_PROJECT_NAME:-wikijs}"
DOCKER_COMPOSE_FILE="${DOCKER_COMPOSE_FILE:-wikijs-traefik-letsencrypt-docker-compose.yml}"
...
test_restore_roundtrip() {
  ...
  echo "  restoring the baseline with ./wikijs-restore-database.sh"
  COMPOSE_PROJECT_NAME="$COMPOSE_PROJECT_NAME" ./wikijs-restore-database.sh "$(basename "$baseline")" \
