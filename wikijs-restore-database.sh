#!/usr/bin/env bash
# wikijs-restore-database.sh [backup-file-name]
#
# Replaces the Wiki.js database with one of the dumps the backups service wrote.
#
#   ./wikijs-restore-database.sh               list and ask
#   ./wikijs-restore-database.sh <file-name>   restore that one
#
...
#
# Set COMPOSE_PROJECT_NAME if the stack was started with a -p other than wikijs.
set -Eeuo pipefail

PROJECT="${COMPOSE_PROJECT_NAME:-wikijs}"
APP_SERVICE="wikijs"
...
DB_NAME="$(env_of WIKIJS_DB_NAME)"; DB_USER="$(env_of WIKIJS_DB_USER)"
...
if ! docker exec "$BKP" sh -c "(set -o pipefail) 2>/dev/null && set -o pipefail; set -eu
  ...
  echo "error: the restore failed part-way. The database may now be empty: restore another backup before using Wiki.js." >&2
  exit 1
fi
